...
class BenchmarkDataset(Dataset):
    def __init__(self, meta_file, image_folder,transforms, tokenizer):
        ...
        used_prompts = []
        files = []
        filename_list = natsorted(os.listdir(image_folder))
        for idx, prompt in enumerate(prompts):
            # filename = os.path.join(self.image_folder, f'{idx:05d}.jpg')
            filename = os.path.join(image_folder, filename_list[idx])
...
def evaluate_benchmark(data_path, img_path, model, batch_size, preprocess_val, tokenizer, device):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            score.extend(torch.diagonal(logits_per_image).cpu().tolist())
    ...
    print('{:<15}'.format('Average'), '{:.2f}'.format(np.mean(score)), '\t')
...
model_dict = {}
model_name = "ViT-H-14"
# model_name = '/home/bingxing2/ailab/quyichang/zgh/data/pretrain_model/models--laion--CLIP-ViT-H-14-laion2B-s32B-b79K'
precision = 'amp'
